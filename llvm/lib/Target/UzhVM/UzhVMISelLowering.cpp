...
#include "UzhVM.h"
#include "UzhVMMachineFunctionInfo.h"
...
#define DEBUG_TYPE "UzhVM-lower"

using namespace llvm;

static const MCPhysReg ArgGPRs[] = {UzhVM::R9, UzhVM::R10, UzhVM::R11, UzhVM::R12};

void UzhVMTargetLowering::ReplaceNodeResults(SDNode *N,
                                           SmallVectorImpl<SDValue> &Results,
                                           SelectionDAG &DAG) const {
  llvm_unreachable("");
}

UzhVMTargetLowering::UzhVMTargetLowering(const TargetMachine &TM,
                                     const UzhVMSubtarget &STI)
    : TargetLowering(TM), STI(STI) {
  UZHVM_DUMP_RED
  addRegisterClass(MVT::i32, &UzhVM::GPRRegClass);
  computeRegisterProperties(STI.getRegisterInfo());

  setStackPointerRegisterToSaveRestore(UzhVM::R1);
...
const char *UzhVMTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  switch (Opcode) {
  case UzhVMISD::CALL:
    return "UzhVMISD::CALL";
  case UzhVMISD::RET:
    return "UzhVMISD::RET";
  case UzhVMISD::BR_CC:
    return "UzhVMISD::BR_CC";
...
#include "UzhVMGenCallingConv.inc"
...
SDValue UzhVMTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_UzhVM);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, UzhVM::R1, PtrVT);
      SDValue Address =
  ...
  Chain = DAG.getNode(UzhVMISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_UzhVM);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
                                   ...
                                   const UzhVMSubtarget &Subtarget) {
...
/// UzhVM formal arguments implementation
SDValue UzhVMTargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_UzhVM);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &UzhVM::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    UzhVMFunctionInfo *UFI = MF.getInfo<UzhVMFunctionInfo>();
...
bool UzhVMTargetLowering::CanLowerReturn(
  ...
  if (!CCInfo.CheckReturn(Outs, RetCC_UzhVM))
    return false;
...
SDValue
UzhVMTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
  ...
  const UzhVMSubtarget &STI = MF.getSubtarget<UzhVMSubtarget>();
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_UzhVM);
  ...
  return DAG.getNode(UzhVMISD::RET, DL, MVT::Other, RetOps);
}
...
SDValue UzhVMTargetLowering::PerformDAGCombine(SDNode *N,
...
bool UzhVMTargetLowering::isLegalAddressingMode(const DataLayout &DL,
